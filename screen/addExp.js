...
const addExpense = ({ route, navigation }) => {
  ...
  const [showcamrea, setshowCamera] = useState(''); 
  ...
  const ckeckPermetion = async () => {
    const cameraPermission = Camera.getCameraPermissionStatus();
    const permission = await Camera.requestCameraPermission();
    console.log(`camera permission status: ${permission}`)
    if (permission === 'denied') await Linking.openSettings()
    const microphonePermission = Camera.getMicrophonePermissionStatus()
    console.log(cameraPermission, "camera permistion ======================================");
  }
  ...
  takePicture = async () => {
    ...
    try {
      ...
      var joined = { camUri: data.path, randonNum: Math.floor(100000 + Math.random() * 900000) }
    ...
    <View style={style.container}>
          ...
          <View style={{
            ...
            <TouchableOpacity onPress={() => takePicture()} >
              <View onPress={() => takePicture()} style={{
                position: "relative", backgroundColor: "#000000", justifyContent: 'center', borderColor: "#ffffff", borderWidth: 2,
      ...
      <ScrollView>
        <View style={style.labelinput}>
          ...
          <View style={{ paddingLeft:10, width: "88%", height: 40, }}>
            {/* <DropDownPicker
              items={expenseType}
 ...
 <Dropdown
      ...
      value={data.selectedExpenseType}
      onChange={item => {
        console.log("ITEM ==== ", item.value);
                data.selectedExpenseType = item.value
                console.log(" data.selectedExpenseType====" + data.selectedExpenseType)
