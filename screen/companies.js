...
import { View, Text, FlatList, ActivityIndicator,BackHandler,Dimensions,Animated, TouchableOpacity,useRoute } from 'react-native';
...
handleBackButton = () => {
  ...
  if(currentRoute =='companies' || currentRoute =='HomeOne'){
  ...
  makeRemoteRequest = () => {
    ...
    console.log("URL Company list ========== ",url);
    // return
    ...
    fetch(url)
      ...
      .then(res => {
       ...
       if(res.ResultInfo.length ==1){
        ...
        let CompanyName =res.ResultInfo[0].CompanyName.toString();
        let companyCode =res.ResultInfo[0].CompanyCode.toString();
        AsyncStorage.setItem('companyCode',companycodes);
        AsyncStorage.setItem('companyName',CompanyName);
        AsyncStorage.setItem('companycodee',companyCode);
        console.log("companyName======================"+CompanyName);
        if (result.UserType==='Employee') {
          this.props.navigation.navigate('HomeOne');
          }
        else if (result.UserType==='Employer') {
          this.props.navigation.navigate('EmployerLogin');
          // this.props.navigation.navigate('employerFooter');
  ...
  setCompanyCode = (val) => {
    AsyncStorage.getItem('userDetails').then(res => { // result.UserType data comes from userDetails
    var result = new Array();
          result =JSON.parse(res);
    // let companycodes =val.CompanyCode.toString();
    let companycodes =val.PayrollCompanyCode.toString();
    AsyncStorage.setItem('companyCode',companycodes);
    AsyncStorage.setItem('companyName',val.CompanyName.toString()); // companyName show in employerview
